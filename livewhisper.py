...
class StreamHandler:
   def __init__(self, assist=None):
       ...
       sys.stdout.write("\033[96mLoading Whisper Model..\033[0m")
       sys.stdout.flush()
       #self.model = whisper.load_model(f'{Model}{".en" if English else ""}')
       #print("\033[90m Done.\033[0m")
       self.model = whisper.load_model('{}{}'.format(Model, ".en" if English else ""))
       sys.stdout.write("\033[90m Done.\033[0m\n")
   ...
   def callback(self, indata, frames, time, status):
       ...
       if np.sqrt(np.mean(indata**2)) > Threshold and Vocals[0] <= freq <= Vocals[1] and not self.asst.talking:
           #print('.', endfrom flask import Flask, jsonifyh()
   ...
   def process(self):
       if self.fileready:
           ...
           formatted_string = "\033[1A\033[2K\033[0G%s" % result['text']
